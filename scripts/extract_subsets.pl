...
# Obtain the directory of the subsets file
my($file, $dir, $ext) = fileparse($subsets);

my $subset_file_out = $dir . "tmp.subsets";
...
foreach my $line (@lines)
{
	...
	my $subset_r = $dir . "subsets.$i";  # for mpest.
	open(OUT1, ">", $subset_r) or die "can't open $subset_r: $!";
	
	my $specieslist = $dir . "specieslist.$i";
