...
sub badInput {
  my $message = "Usage: perl $0
	...
	-od=<output dir (FULL PATH). the subsets will be created in this directory>
	-outgrp=<outgroups>
...
my $dcm = "/projects/sate7/tools/dactal-ship.v1/lisan_dcm/src";
...
my $s = $max_subset_size - 2;  # it ensures the subsetsize. you can substract 2,3,4 etc depending on your datasets
...
for ($i = 1; $i <= $n_subsets; $i++)
	{
		...
		`python $perl_scripts/induced_subtree_from_taxa.py $inFile $subset_File_s`;
		`python $perl_scripts/induced_subtree_from_taxa.py $inFile $subset_File_r`;
		#last;
	}
...
`mkdir $phylo_dir` if not -e $phylo_dir;
...
########################################3333 ARBITRARY RESOLVING THE SUPERFINE OUTPUT #########################
