...
foreach $line (@lines)
	...
	if ((is_present(\@methods, "mpest") == 1)||(is_present(\@methods, "phylonet-heu") == 1)||(is_present(\@methods, "astral") == 1)){
	...
	if ((is_present(\@methods, "phylonet-heu") == 1)||(is_present(\@methods, "astral") == 1)){
		foreach $taxa (@taxa)
		{
		print OUT1 "$taxa\n";
		#print OUT2 "$taxa 1 $taxa\n";	# species list is only for relaxed subsets. since we run mpest on relaxed subsets.
...
# else dieo kora jai. karon ekmatro exact er khetrei strict dorkar
		if ((is_present(\@methods, "mpest") == 1)||(is_present(\@methods, "phylonet-heu") == 1)||(is_present(\@methods, "astral") == 1)){`python $perl_scripts/induced_subtree_from_taxa.py $inFile $subset_File_r`;}
		#last;
	}
