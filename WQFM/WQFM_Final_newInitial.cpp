...
int noOfVat;
int noOfDef;
double totSatW, totVioW, totDefW;
...
double Calculate_Score(taxa *parta, taxa *partb, quartet *qt, string tempTaxa, int st, int vt, int df, double wst, double wvt, double wdf)
{
	double partitionScore;
	int qscore;
	double dps, dps1;
	quartet *q;
	int s, v,l,d;
	double ws, wv, wd;
	ws = 0.0; wv = 0.0; wd = 0.0;
	s = 0; v = 0; d = 0;
	char c;
	double p;
...
//	cout<<"Start of calcscore\n";
	...
	while(q!= NULL){
		if(!tempTaxa.compare("NULL"))
		{
			...
			if(qscore == 6) {s++; ws+=q->weight;}
			else if(qscore == 2) {v++; wv+= q->weight;}
			else if(qscore == 3) {d++; wd+= q->weight;}
		}
		...
		{	qscore  = Check_Quartet(parta, partb, q, tempTaxa); 
			...
			if(c=='s' && qscore == 2) { s--; v++; ws-=q->weight; wv+=q->weight;} // s v
					
			else if(c=='s' && qscore==3){ s--; d++; ws-=q->weight; wd+=q->weight;} // s d
					
			else if(c=='v' && qscore == 6){v--; s++; wv-=q->weight;ws+=q->weight;} // v s		
					
			else if(c=='v' && qscore==3){v--;d++; wv-=q->weight; wd+=q->weight;}  // v d

			else if(c=='d' && qscore==2){d--;v++; wd-=q->weight; wv+=q->weight;} // d v 

			else if(c=='d' && qscore==6){d--;s++; wd-=q->weight; ws+=q->weight;} // d s

			else if(qscore==1)
			{
				if(c=='s') {s--; ws-=q->weight;}
				else if(c=='v') {v--; wv-=q->weight;}
				else if(c=='d') {d--; wd-=q->weight;}
...
taxa* FM_Algo(taxa* partA, taxa* partB, quartet* q)
{
	...
	double prevws, prevwd, prevwv; //weight
	string qtemp;
	string TaxaToMove;
	//int minTaxaScore;
	bool dontmove = false;
	...
	{   loop++;
		...
		while(iterationMore){
			...
			score = Calculate_Score(partA, partB,q,"NULL",0,0,0,0.0,0.0,0.0); // sending just 1 partition is OK to find score , initial score //Changed
			ca = CountPart(partA);
			cb = CountPart(partB);
			total  = ca+cb;
			prevs = noOfSat; prevd = noOfDef;prevv = noOfVat;
			prevws = totSatW; prevwd = totDefW; prevwv = totVioW; //weight

			prevscore = score; // weighted score
			...
			while(flag)
			{
				...
				if(alt == 0 && tag1 ==0 ){
					...
					{	tag1 = 1;
						...
						score = Calculate_Score(partA,partB,q, TaxaToMove,prevs,prevv,prevd,prevws,prevwv,prevwd); //weight
						gl = gainList;
						while(gl->next!=NULL)
							gl = gl->next;
						list *gtemp;
						...
						gtemp->val = score - prevscore; //weighted
						gtemp->below =  ca - 1; 
						gtemp->sat = noOfSat;
						gtemp->vat = noOfVat;
						gtemp->def = noOfDef;
						gtemp->wsat = totSatW; //weight
						gtemp->wvat = totVioW; //weight
						gtemp->wdef = totDefW; //weight
			...
			gl = gainList->next;
			double maxgain = -1000000000000000000.00; //int
			int minvio = 0;
			int maxsat = 0;
			double maxwsat = 0;
			int glPart = 0;
			int randnum = 0;
			long int maxdiff = -10000000000000000, tempdiff;
			long double maxdiffw = -10000000000000000.00;
			...
			while(gl!=NULL)
			{
				...
				{ 
					...
					maxdiffw = gl->wsat - gl->wvat; //weight
					
					maxsat = gl->sat;
					maxwsat = gl->wsat; //weiht
				...
				{ 
					...
					if(gl->wsat > maxwsat && gl->below>=2)// && ((c1>2||c2>2)&& total!=gl->val+gl->sat)) //(tempratio1>maxratio1)
					{
						TaxaToMove = gl->str;
						maxgain = gl->val;
						maxsat = gl->sat;
						maxwsat = gl->wsat; //weight
						maxdiff = gl->sat - gl->vat;
						maxdiffw = gl->wsat - gl->wvat; //weight
						glPart = gl->part; 
					}
					else if(gl->wsat == maxwsat && gl->below>=2)// &&((c1>2||c2>2)&& total!=gl->val+gl->sat))//(tempratio1==maxratio1)
					{	
						tempdiff = gl->sat-gl->vat;

						if(tempdiff>maxdiff && gl->below>=2 )//&&((c1>2||c2>2)&& total!=gl->val+gl->sat))
						{ 
							TaxaToMove = gl->str;
							maxgain = gl->val;
						...
						else if(tempdiff==maxdiff && gl->below>=2 )//&&((c1>2||c2>2)&& total!=gl->val+gl->sat))
						{ 
							if(gl->sat > maxsat && gl->below>=2)				
							{
								TaxaToMove = gl->str;
								maxgain = gl->val;
							...
							else if(gl->sat == maxsat && gl->below>=2)
							{
								randnum = 10 + rand()%100;
								if(randnum%2==0){
									TaxaToMove = gl->str;
...
taxa* FM(taxa* t, quartet* q)
{
	...
	taxa *partA, *partB, *P, *temp, *pa, *pb, *taxaList,*patemp,*pbtemp,*Prev,*partC, *partD, *pc, *pd,*pctemp,*pdtemp;
	...
	{			
		...
		while(Qtemp->qnext!=NULL)
		{
			Qt  = Qt->qnext;
			Qtemp  = Qtemp->qnext;
			//cout << Qtemp->quartet_id<<" & weight = "<<Qtemp->weight<<" "<<Qt->quartet_id<<" & weight = "<<Qt->weight<<endl;

			if(Qtemp->weight>Qt->weight)
			{
...
//	cout<<"in FM2"<<endl;
	...
	{	which  = -1;
		...
		if(noOfPart==2)
		{
			pctemp = partC;
			pdtemp = partD;
			while(pctemp->tnext!=NULL)
			{
				pctemp = pctemp->tnext;
				if(!(qq1.compare(pctemp->name)&&qq2.compare(pctemp->name)&&qq3.compare(pctemp->name)&&qq4.compare(pctemp->name)))
				{
					if(!qq1.compare(pctemp->name)) a1 = 0;
					else if (!qq2.compare(pctemp->name)) b1 = 0;
					else if (!qq3.compare(pctemp->name)) c1 = 0;
					else if (!qq4.compare(pctemp->name)) d1 = 0;
				}
			}
			while(pdtemp->tnext!=NULL)
			{
				pdtemp = pdtemp->tnext;
				if(!(qq1.compare(pdtemp->name)&&qq2.compare(pdtemp->name)&&qq3.compare(pdtemp->name)&&qq4.compare(pdtemp->name)))
				{
					if(!qq1.compare(pdtemp->name)) a1 = 1;
					else if (!qq2.compare(pdtemp->name)) b1 = 1;
					else if (!qq3.compare(pdtemp->name)) c1 = 1;
					else if (!qq4.compare(pdtemp->name)) d1 = 1;
				}
			}
			
		}
		
		if(!(a==-1 && b==-1 && c==-1 && d==-1) && !(a1==-1 && b1==-1 && c1==-1 && d1==-1)) which = 0; //2 ta tei atleast 1 ta kore ase
		else if(!(a==-1 && b==-1 && c==-1 && d==-1) ) which = 1; //atleast 1ta 1 e ase
		else if(!(a1==-1 && b1==-1 && c1==-1 && d1==-1) ) which = 2; //atleast 1 2 e ase
		else which = 3; //2 ta tei nai


		if(which == 3) //all new
		{
			if(NoOfPart==0){ //this is the first quartet
				NoOfPart = 1;
				temp = new taxa();
				temp->name = Qt->q1;
				temp->partition = 0;
			...
			else {         //for the 1st quartet with all new taxa when we already have 1 or 2 partitions, so it randomly to the 2nd partition

				if(NoOfPart==1) NoOfPart = 2;
		...
		else if(which == 2)
		{
			...
			{	temp = new taxa();
				temp->name = Qt->q1;
				if(b1!=-1){
					if(b1==0) {temp->partition = 0; pc->tnext = temp; pc= pc->tnext; a1=0;ca++;}
					else {temp->partition = 1; pd->tnext = temp; pd= pd->tnext; a1=1;cb++;}
				}
				else if(c1!=-1){
					if(c1==1) {temp->partition = 0; pc->tnext = temp; pc= pc->tnext; a1=0;ca++;}
					else {temp->partition = 1; pd->tnext = temp; pd= pd->tnext; a1=1;cb++;}
				}
				else if(d1!=-1)
				{
					if(d1==1) {temp->partition = 0; pc->tnext = temp; pc= pc->tnext; a1=0;ca++;}
					else {temp->partition = 1; pd->tnext = temp; pd= pd->tnext; a1=1;cb++;}
				}
			...
			if(b1==-1)
			{
				...
				if(a1==0) {temp->partition = 0; pc->tnext = temp; pc= pc->tnext; b1=0;ca++;}
				else {temp->partition = 1; pd->tnext = temp; pd= pd->tnext; b1=1;cb++;}
			...
			if(c1==-1)
			{
				...
				if(d1!=-1)
				{
					if(d1==0) {temp->partition = 0; pc->tnext = temp; pc= pc->tnext; c1=0;ca++;}
					else {temp->partition = 1; pd->tnext = temp; pd= pd->tnext; c1=1;cb++;}
				}
				else{
					if(a1==1) {temp->partition = 0; pc->tnext = temp; pc= pc->tnext; c1=0;ca++;}
					else {temp->partition = 1; pd->tnext = temp; pd= pd->tnext; c1=1;cb++;}
				}
			}
			if(d1==-1)
			{
				temp = new taxa();
				temp->name = Qt->q4;
				if(c1==0) {temp->partition = 0; pc->tnext = temp; pc= pc->tnext; d1=0;ca++;}
				else {temp->partition = 1; pd->tnext = temp; pd= pd->tnext; d1=1;cb++;}
			}
		
		}
		else if(which == 0)
		{	
                  	int merge = 0; //1 merge C to qr[indx1], 2 merge D to qr[indx1]
			int indx1 = -1; int indx2 = -1;
			int qr[4]; qr[0]=a; qr[1]=b;qr[2]=c;qr[3]=d;
			int qr1[4]; qr1[0]=a1; qr1[1]=b1;qr1[2]=c1;qr1[3]=d1;
			for(int i= 0; i<4; i++){ if(qr[i]!=-1){indx1 = i; break;}}
			for(int i= 0; i<4; i++){ if(qr1[i]!=-1){indx2 = i; break;}}
		
			if((indx1<2&&indx2<2)||(indx1>=2&&indx2>=2)) //same side
			{
				if(qr1[indx2]==0) merge = 1;//merge C to qr[indx1]
				else merge = 2;//merge D to qr[indx1]
			}
			else{ //opposite side
				
				if(qr1[indx2]==0) merge = 2;//merge D to qr[indx1]
				else merge = 1;//merge C to qr[indx1]
			}
		  	if(merge==1)
			{
				pctemp = partC;
				...
				while(pdtemp->tnext!=NULL)
				{
					...
					if(qr[indx1]==0)
					pdtemp->partition = 1;
					else pdtemp->partition = 0;
				}


				if(qr[indx1]==0){ pa->tnext = partC->tnext; pb->tnext = partD->tnext;}
				else { pa->tnext = partD->tnext; pb->tnext = partC->tnext;}
			}
			else if(merge==2)
			{
				...
				while(pdtemp->tnext!=NULL)
				{
					...
					if(qr[indx1]==0)
					pdtemp->partition = 0;
					else pdtemp->partition = 1;
				}


				if(qr[indx1]==1){ pa->tnext = partC->tnext; pb->tnext = partD->tnext;}
				else { pa->tnext = partD->tnext; pb->tnext = partC->tnext;}
			}
			partC = new taxa();
			pc = partC;
...
void readQuartets()
{
	...
	//	cout<<"Quartet "<<count<<endl;
		...
		if(!temp->q2.compare("0"))
			temp->q2.assign("O");
		try{
		qt.assign(qt.replace(0,found+3,""));
		}
		...
		cin>>qt; //this is my weight //cout<<qt<<endl;
		double numb;
		istringstream ( qt) >> numb; 
		//cout<<numb<<endl;
		temp->weight = numb;
		//temp->weight = 1;

		//cout<< numb<<endl;
		//cout<<temp->q1<<","<<temp->q2<<"|"<<temp->q3<<","<<temp->q4<<endl;

		loq = listOfquartet; 
		while(loq->qnext!=NULL)
		{	m = loq->qnext;
			if(m->weight>=temp->weight) loq = loq->qnext;
			else{
				temp->qnext = m;
				loq->qnext = temp;
				break;
...
string merge(string s1, string s2, string extra)
{
	...
	while(pFile==NULL){ fclose(pFile);  pFile = fopen(buffer, "r");  }
	char c;
...
int main(int argc, char *argv[])
{
	...
	while(loq->qnext!=NULL){
		loq = loq->qnext;
		cout<<loq->q1<<","<<loq->q2<<"|"<<loq->q3<<","<<loq->q4<<" "<<loq->weight<<endl;
	}*/
